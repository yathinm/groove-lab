  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-orange-200 bg-white p-3">
      ...
      <div className="flex-1 min-w-0">
        <strong className="text-slate-900">Original</strong>
        <span className="ml-2 text-xs text-slate-600">{!hasOriginal ? 'Unavailable' : (isPlayingThis ? 'Now playing' : 'Ready')}</span>
      </div>
    </div>
  )
}
